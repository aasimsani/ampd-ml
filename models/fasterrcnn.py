...
from torchvision.models.detection.backbone_utils import (
    resnet_fpn_backbone, _validate_trainable_layers, mobilenet_backbone
    )

# Customized for mean average precision
from .fasterrcnn_custom.roi_heads import RoIHeads
from .fasterrcnn_custom.generalized_rcnn import GeneralizedRCNN
from .fasterrcnn_custom.rpn import RPNHead, RegionProposalNetwork
...
class FasterRCNN(GeneralizedRCNN):
    ...
    returns the post-processed predictions as a List[Dict[Tensor]], one for
    ...
    Args:
        backbone (nn.Module): the network used to compute the 
            features for the model. It should contain a out_channels attribute,
            which indicates the number of output channels that each feature map
            has (and it should be the same for all feature maps).
            The backbone should return a single Tensor or 
            and OrderedDict[Tensor].
        ...
        max_size (int): maximum size of the image to be rescaled before 
                        feeding it to the backbone
        image_mean (Tuple[float, float, float]): mean values used for 
            input normalization. They are generally the mean values of
            the dataset on which the backbone has been trained on
        image_std (Tuple[float, float, float]): std values used for
            input normalization. They are generally the std values of 
            the dataset on which the backbone has been trained on
        rpn_anchor_generator (AnchorGenerator): module that generates 
            the anchors for a set of feature maps.
        rpn_head (nn.Module): module that computes the objectness and 
            regression deltas from the RPN
        rpn_pre_nms_top_n_train (int): number of proposals to keep 
            before applying NMS during training
        rpn_pre_nms_top_n_test (int): number of proposals to keep
            before applying NMS during testing
        rpn_post_nms_top_n_train (int): number of proposals to keep
            after applying NMS during training
        rpn_post_nms_top_n_test (int): number of proposals to keep after
            applying NMS during testing
        rpn_nms_thresh (float): NMS threshold used for postprocessing
            the RPN proposals
        rpn_fg_iou_thresh (float): minimum IoU between the anchor and 
            the GT box so that they can be considered as positive during
            training of the RPN.
        rpn_bg_iou_thresh (float): maximum IoU between the anchor and
            the GT box so that they can be considered as negative during training of the RPN.
...
class FastRCNNPredictor(nn.Module):
    ...
    def __init__(self, in_channels, num_classes, dropout_logits, dropout_box):
        super(FastRCNNPredictor, self).__init__()
        self.cls_score = nn.Linear(in_channels, num_classes)
        self.bbox_pred = nn.Linear(in_channels, num_classes * 4)
        self.dropout_logits = nn.Dropout(dropout_logits)
        self.dropout_box = nn.Dropout(dropout_box)
    ...
    def forward(self, x):
        ...
        x = x.flatten(start_dim=1)
        x_scores = self.dropout_logits(x)
        scores = self.cls_score(x_scores)
        x_box = self.dropout_box(x)
